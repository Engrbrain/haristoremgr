...
router.post(`/`, async (req, res) => {
    ...
    const orderItemsIds = await Promise.all(req.body.map(async orderitem => { 
        let newOrderItem = new OrderItem({
            quantity: orderItem.quantity,
            product: orderItem.product
        })

        newOrderItem = await new OrderItem.save();
...
router.put('/:id', async (req, res)=> {
    ...
    if(!order)
    return res.status(404).send('The Order Status cannot be updated at this time')
...
//get order count for statistical purpose
